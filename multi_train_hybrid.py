...
def select_hybrid_mode():
    print("\nSelect hybrid mode:")
    print("  1. DDT as a classifier")
    print("  2. RL as a classifier")
    while True:
        choice = input("Enter choice [1/2]: ").strip()
        if choice == '1':
            print("Selected: DDT as a classifier")
            return 'classifier'
        elif choice == '2':
            print("Selected: RL as a classifier")
            return 'rl'
...
def main():
    ...
    for mode, data_fn in selected_datasets:
        SAVE_DIR = f'./training_results/results_multi_run_ddt_{HYBRID}/{mode}'
        METRICS_FILE = os.path.join(SAVE_DIR, 'all_runs_metrics.csv')
        print(f"Using the DDT as {HYBRID}")
