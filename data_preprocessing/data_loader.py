...
class Data_Loader():
        ...
        def train_test_split_onto_4(self):
                ...
                part 4: test 10%, final evaluation here (no training or hyperpara tuning dependent)
                '''
                # [FIX] Stratify by labels (last column) to ensure equal class distribution
                self.train, self.test = train_test_split(self.data, test_size=0.1, random_state=7, stratify=self.data[:, -1])  # self.test: 10%

                # [FIX] Stratify these splits too
                self.train, self.train_rl = train_test_split(self.train, test_size=1/3, random_state=14, stratify=self.train[:, -1]) 
                self.train_rl, self.val_rl = train_test_split(self.train_rl, test_size=1/6, random_state=21, stratify=self.train_rl[:, -1]) # train for rl: 25%, val for rl: 5%
        ...
        def random_augment(self, data, M = 1):
                '''
                Vectorised version: builds all masks via numpy broadcasting
                ...
                size = (m + 1) * N * M
                complete_data = np.tile(data, (m + 1, M, 1)).reshape(size, -1)
                mask = np.zeros_like(complete_data)
                missing_data = complete_data.copy()

                # Pre-compute block column indices
                block_cols = [self.block[k + 1] for k in range(m)]

                count = 0
                for n_obs in range(m + 1):
                        prob = n_obs / m if m > 0 else 0
                        chunk = N * M
                        # Generate all random thresholds at once for this n_obs level
                        rands = np.random.rand(chunk, m)
                        unobs_flags = rands < prob  # True where panel is unobserved

                        for j in range(chunk):
                                for k in range(m):
                                        if unobs_flags[j, k]:
                                                cols = block_cols[k]
                                                missing_data[count + j, cols] = np.nan
                                                mask[count + j, cols] = 1.0
                        # Also mark label column in mask if it was NaN (shouldn't be)
                        # mask for label column is handled by slice below
                        count += chunk
